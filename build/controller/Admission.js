...
const createAdmission = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        const { patients_id, admission_date, admission_room_number, ailment, business_id } = req.body;
        const { error, value } = admissionValidation_1.admissionSchema.validate(req.body);
        if (error) {
            return res.status(400).json({ message: error.details[0].message });
        }
        const data = { patients_id, admission_date, admission_room_number, ailment, business_id };
